...
// Strictly follow the rule: Use process.env.GEMINI_API_KEY directly in the constructor.
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

export const getShoppingAssistantResponse = async (userQuery: string, cartItems: string[], currentProducts: Product[]) => {
...
export const getStyleHubRecommendations = async (styleDescription: string, currentProducts: Product[]) => {
  const model = "gemini-3-flash-preview";
  ...
  try {
    ...
    return currentProducts.filter(p => ids.includes(p.id));
  } catch (error) {
