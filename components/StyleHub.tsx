...
export const StyleHub: React.FC<StyleHubProps> = ({ onAddToCart, products }) => {
  ...
  const quickVibes = [
    ...
    { label: '🛒 Weekly Pantry Restock', value: 'Fresh organic groceries, essential household items, and healthy snacks for a busy family week.' },
  ];
  ...
  const handleConsult = async (customVibe?: string) => {
    ...
    setLoading(true);
    const results = await getStyleHubRecommendations(finalVibe, products);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-32">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="grid grid-cols-1 gap-5">
            {quickVibes.map((qv: { label: string, value: string }, i: number) => (
              <button
                key={i}
