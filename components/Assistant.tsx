...
import { ChatMessage, GroundingSource, Product } from '../types';
...
export const Assistant: React.FC<AssistantProps> = ({ cartItemIds, products }) => {
  ...
  const handleSend = async () => {
    ...
    const result = await getShoppingAssistantResponse(userMsg, cartItemIds, products);
                ...
                {m.sources && m.sources.length > 0 && (
                  <div className="mt-3 max-w-[85%] flex flex-col gap-2 p-4 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-900/30">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {m.sources.slice(0, 3).map((source: GroundingSource, sIdx: number) => (
                        <a 
                          key={sIdx}
                          href={source.uri} 
                          target="_blank" 
